...
def SeqWeights(arr):
	...
	if len(arr2) > 1:
		arr2 = '['+arr2+']'
	return Counter(arr1), arr2, str(count2)+'/'+str(count1)
...
motif_seq, freq = Motifs(arr)
print motif_seq

res_list = sorted(list(res_dict))
legend = []
for r in res_list:
...
for i,pos in enumerate(freq):
	row = [str(i+1)]
	for res in res_list:
		row.append(str(pos[res]/maxn))
